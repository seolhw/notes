...
const type = (a) => {
  const type = Object.prototype.toString.call(a).split(' ')[1]
  return type.substr(0, type.length - 1)
}
// console.log(type('123'))

const compareVersion1 = (a, b) => {
  ...
  for (let i = 0; i < len; i++) {
    const aNum = parseInt(aVersion[i])
    const bNum = parseInt(bVersion[i])
    if (aNum > bNum) {
      return 1
    } else if (aNum < bNum) {
      return -1
...
const compareVersion2 = (a, b) => {
  const aVersion = a.split('.')
  const bVersion = b.split('.')
  while (aVersion.length || bVersion.length) {
    const aNum = parseInt(aVersion.shift())
    const bNum = parseInt(bVersion.shift())
    if (aNum > bNum) {
      return 1
    } else if (aNum < bNum) {
      return -1
